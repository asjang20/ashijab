@props(['store' => null])
<div id="confirm-store" tabindex="-1" aria-hidden="true"
                            ...
                            <li>
                                <div class="flex gap-3">
                                    <div class="flex gap-3 w-full border rounded-lg p-3">
                                        <img src="{{ asset('storage/images/store') }}/{{ $item->logo }}"
                                            class="w-20 h-20 object-cover rounded-lg" alt="">
                                        <div class="flex flex-col justify-between">
                                            <p class="text-lg font-medium">{{ $item->name }}</p>
                                            <p class="text-start line-clamp-2">{{ $item->address }}</p>
                                        </div>
                                    </div>
                                    <form action="{{ route('store.confirm', $item->id) }}" method="POST">
                                        @method('PUT')
                                        @csrf
                                        <button type="submit"
